...
class SetCriterion_Crowd(nn.Module):
    ...
    def loss_entropy(self, outputs, targets, indices, num_points):
        """
        ...
        n, l, c = src_logits.size()
        src_logits = F.softmax(src_logits, -1)
        loss_ent = -torch.sum(torch.mul(src_logits, torch.log2(src_logits + 1e-30))) / (
            n * l * np.log2(c)
        )

        losses = {"loss_entropy": loss_ent}
        return losses
    ...
    def forward(self, outputs, targets, source=1):
        ...
        if source == 1:
            ...
            for loss in self.losses[:2]:
                losses.update(
                    self.get_loss(loss, output1, targets, indices1, num_boxes)
                )
        else:
            losses.update(self.loss_entropy(output1, targets, None, None))
...
def build(args, training):
    ...
    weight_dict = {"loss_ce": 1, "loss_points": args.point_loss_coef, "loss_entropy": 0.05}
    losses = ["labels", "points", "entropy"]
